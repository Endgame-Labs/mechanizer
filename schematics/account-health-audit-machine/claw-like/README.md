...
2. Runner acquires non-blocking distributed lock (claw.account_health_audit); if lock exists, mark skipped_overlap and exit.
3. Load cursor window (lookback: 72h) and build deterministic candidate set.
...
- Schedule: 0 6 * * 1-5 (America/Los_Angeles)
- Cadence note: Weekday morning audit with a wide lookback to catch weekend drift and controller downtime.
- Cursor lookback: 72h
- Grace window: 600 seconds
- Stale threshold: 129600 seconds
...
- Emit diagnostics event: account.health.audit.machine.safe_mode
