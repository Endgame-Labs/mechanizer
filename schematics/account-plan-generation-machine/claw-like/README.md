...
2. Runner acquires non-blocking distributed lock (claw.account_plan_generation); if lock exists, mark skipped_overlap and exit.
3. Load cursor window (lookback: 90m) and build deterministic candidate set.
...
- Cadence note: Half-hour cadence balances freshness and batch cost.
- Cursor lookback: 90m
- Grace window: 180 seconds
- Stale threshold: 5400 seconds
...
- Dead-letter sink: dlq.account-plan-generation
- DLQ payload minimum fields: machine_id, run_id, event_id, stage, error_code, retry_count, first_seen_at.

## HITL and Risk Controls
- Risky actions requiring approval: crm_plan_publish, outbound_plan_distribution
...
- Emit diagnostics event: account.plan.machine.safe_mode
