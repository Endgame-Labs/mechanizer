# Agentic Adapter (account-plan-generation-machine)

Runtime-agnostic orchestration contract for `account-plan-generation-machine` using a **planner -> executor -> evaluator** loop with deterministic event I/O.

## Purpose
Generate actionable account plans (whitespace and plays) with controlled mutations and approval-first operationalization.
...
Any CRM plan writeback, task creation, or outbound distribution is HITL-gated.
