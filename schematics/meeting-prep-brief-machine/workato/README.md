# Workato Adapter (meeting-prep-brief-machine)

![Meeting Prep Brief Machine Diagram](../diagram.svg)

Workato implementation notes for `meeting-prep-brief-machine` using API recipes, recipe functions for shared smart cogs, lookup-table idempotency, and Workato job history for replay/debugging.

## Artifact
- `recipe.json`: Workato recipe reference scaffold for build guidance and lifecycle packaging.

## Recipe Shape
1. Receive `gtm_event_v1` through an API Platform recipe endpoint for `meeting-prep-brief-machine`.
2. Validate and normalize the canonical event envelope.
3. Check a lookup-table idempotency key before side effects.
4. Call shared recipe functions for smart cogs: `enrich_account_health`, `deal_score_reasoner`, `directive_alignment`, `compose_outreach_message`, `route_exec_alert`, `approval_loop`.
...
## Terminal Events
- Approved path emits `meeting.prep_brief.delivered` from `emit_terminal_event.body.event_type`.
- Blocked/deferred path emits `meeting.prep_brief.blocked` from `emit_terminal_event.body.event_type`.
## Idempotency and State
- Use a lookup table key `meeting-prep-brief-machine:{event_id}` before any downstream side effect.
