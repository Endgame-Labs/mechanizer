...
2. Runner acquires non-blocking distributed lock (claw.new_hire_ramp); if lock exists, mark skipped_overlap and exit.
...
- Cadence note: 20-minute cadence provides timely ramp signals while limiting noisy reprocessing.
- Cursor lookback: 2h
...
- Dead-letter sink: dlq.new-hire-ramp-accelerator
- DLQ payload minimum fields: machine_id, run_id, event_id, stage, error_code, retry_count, first_seen_at.

## HITL and Risk Controls
- Risky actions requiring approval: hris_profile_write, external_manager_email_send
...
- Emit diagnostics event: ramp.machine.safe_mode
