...
## Recipe Shape
1. Receive `gtm_event_v1` through an API Platform recipe endpoint for `new-hire-ramp-accelerator-machine`.
2. Add a companion scheduled recipe for `0 */4 * * *` in `workspace default` that calls the same recipe-function path for periodic sweeps.
3. Validate and normalize the canonical event envelope.
4. Check a lookup-table idempotency key before side effects.
5. Call shared recipe functions for smart cogs: `enrich_account_health`, `directive_alignment`, `compose_outreach_message`, `route_exec_alert`, `approval_loop`.
...
## Terminal Events
- Approved path emits `rep.onboarding.package_generated` from `emit_terminal_event.body.event_type`.
- Blocked/deferred path emits `rep.onboarding.package_blocked` from `emit_terminal_event.body.event_type`.
## Idempotency and State
- Use a lookup table key `new-hire-ramp-accelerator-machine:{event_id}` before any downstream side effect.
