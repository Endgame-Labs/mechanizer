...
2. Runner acquires non-blocking distributed lock (claw.renewal_risk_monitoring); if lock exists, mark skipped_overlap and exit.
...
- Cadence note: Daily cadence supports playbook-driven renewal operations.
- Cursor lookback: 48h
...
- Dead-letter sink: dlq.renewal-risk-monitoring
- DLQ payload minimum fields: machine_id, run_id, event_id, stage, error_code, retry_count, first_seen_at.

## HITL and Risk Controls
- Risky actions requiring approval: customer_email_send, crm_task_write, playbook_state_change
...
- Emit diagnostics event: renewal.risk.machine.safe_mode
