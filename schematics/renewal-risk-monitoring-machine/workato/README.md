# Workato Adapter (renewal-risk-monitoring-machine)

Workato implementation notes for `renewal-risk-monitoring-machine` using API recipes, recipe functions for shared smart cogs, lookup-table idempotency, and Workato job history for replay/debugging.

## Artifact
- `recipe.json`: Workato recipe reference scaffold for build guidance and lifecycle packaging.

## Recipe Shape
1. Receive `gtm_event_v1` through an API Platform recipe endpoint for `renewal-risk-monitoring-machine`.
...
## Terminal Events
- Approved path emits `renewal.risk.play.executed` from `emit_terminal_event.body.event_type`.
- Blocked/deferred path emits `renewal.risk.play.blocked` from `emit_terminal_event.body.event_type`.
## Idempotency and State
- Use a lookup table key `renewal-risk-monitoring-machine:{event_id}` before any downstream side effect.
