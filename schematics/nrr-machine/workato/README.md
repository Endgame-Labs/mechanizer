# Workato Adapter (nrr-machine)

![NRR Machine Diagram](../diagram.svg)

Workato implementation notes for `nrr-machine` using API recipes, recipe functions for shared smart cogs, lookup-table idempotency, and Workato job history for replay/debugging.

## Artifact
- `recipe.json`: Workato recipe reference scaffold for build guidance and lifecycle packaging.

## Recipe Shape
1. Receive `gtm_event_v1` through an API Platform recipe endpoint for `nrr-machine`.
