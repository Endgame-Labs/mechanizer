# Workato Adapter (pipeline-review-intelligence-machine)

Workato implementation notes for `pipeline-review-intelligence-machine` using API recipes, recipe functions for shared smart cogs, lookup-table idempotency, and Workato job history for replay/debugging.

## Artifact
- `recipe.json`: Workato recipe reference scaffold for build guidance and lifecycle packaging.

## Recipe Shape
1. Receive `gtm_event_v1` through an API Platform recipe endpoint for `pipeline-review-intelligence-machine`.
2. Add a companion scheduled recipe for `0 7 * * 1` in `America/Los_Angeles` that calls the same recipe-function path for periodic sweeps.
...
## Terminal Events
- Approved path emits `pipeline.review.prep.completed` from `emit_terminal_event.body.event_type`.
- Blocked/deferred path emits `pipeline.review.prep.deferred` from `emit_terminal_event.body.event_type`.
## Idempotency and State
- Use a lookup table key `pipeline-review-intelligence-machine:{event_id}` before any downstream side effect.
