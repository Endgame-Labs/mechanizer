# Workato Adapter (product-propensity-modeling-machine)

Workato implementation notes for `product-propensity-modeling-machine` using API recipes, recipe functions for shared smart cogs, lookup-table idempotency, and Workato job history for replay/debugging.

## Artifact
- `recipe.json`: Workato recipe reference scaffold for build guidance and lifecycle packaging.

## Recipe Shape
1. Receive `gtm_event_v1` through an API Platform recipe endpoint for `product-propensity-modeling-machine`.
2. Add a companion scheduled recipe for `0 */6 * * *` in `America/Los_Angeles` that calls the same recipe-function path for periodic sweeps.
3. Validate and normalize the canonical event envelope.
4. Check a lookup-table idempotency key before side effects.
5. Call shared recipe functions for smart cogs: `propensity_score_reasoner`, `directive_alignment`, `route_exec_alert`, `approval_loop`.
...
## Terminal Events
- Approved path emits `product.propensity.scored` from `emit_terminal_event.body.event_type`.
- Blocked/deferred path emits `product.propensity.blocked` from `emit_terminal_event.body.event_type`.
## Idempotency and State
- Use a lookup table key `product-propensity-modeling-machine:{event_id}` before any downstream side effect.
