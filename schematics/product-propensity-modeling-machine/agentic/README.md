...
## Purpose
Run periodic or event-driven account-product propensity scoring and emit deterministic scores, confidence, reason codes, recommended plays, and evidence refs.

## Runtime Pattern
- **Planner**: validates `gtm_event_v1`, decomposes into tool steps, and sets score path.
...
Read-only scoring can auto-run; any CRM writeback, seller alert, audience sync, or outbound workflow spawned from score output must require HITL in the consuming workflow.
...
- Normalize ChatGPT/Slack requests into `gtm_event_v1` before execution and keep emitted score events contract-identical.
- Use workspace background execution for scheduled or long-running scoring jobs while preserving deterministic checkpoints and replay safety.
