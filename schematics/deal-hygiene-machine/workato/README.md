# Workato Adapter (deal-hygiene-machine)

![Deal Hygiene Machine Diagram](../diagram.svg)

Workato implementation notes for `deal-hygiene-machine` using API recipes, recipe functions for shared smart cogs, lookup-table idempotency, and Workato job history for replay/debugging.

## Artifact
- `recipe.json`: Workato recipe reference scaffold for build guidance and lifecycle packaging.

## Recipe Shape
1. Receive `gtm_event_v1` through an API Platform recipe endpoint for `deal-hygiene-machine`.
...
## Terminal Events
- Approved path emits `deal.hygiene.remediated` from `emit_terminal_event.body.event_type`.
- Blocked/deferred path emits `deal.hygiene.deferred` from `emit_terminal_event.body.event_type`.
## Idempotency and State
- Use a lookup table key `deal-hygiene-machine:{event_id}` before any downstream side effect.
