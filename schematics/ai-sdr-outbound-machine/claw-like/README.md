...
2. Runner acquires non-blocking distributed lock (claw.ai_sdr_outbound); if lock exists, mark skipped_overlap and exit.
...
- Cadence note: High-frequency cadence to keep outreach and response routing near-real-time.
- Cursor lookback: 45m
...
- Dead-letter sink: dlq.ai-sdr-outbound
- DLQ payload minimum fields: machine_id, run_id, event_id, stage, error_code, retry_count, first_seen_at.

## HITL and Risk Controls
- Risky actions requiring approval: sequence_activation, outbound_email_send, crm_owner_write
...
- Emit diagnostics event: sdr.machine.safe_mode
